...
- Role-Based Access Control: Strict separation between Admin and Field Agent roles to ensure data integrity and security.

Assumptions Made
- Reliable Connectivity: The system assumes field agents have internet access when submitting updates.
- Pre-assigned Agents: Fields are manually assigned to agents by the Admin before agents can start monitoring.
-Single Assignee: Each field is assigned to exactly one agent at a time.
...
- Frontend (Vercel): (https://smartseasonfieldsystem.vercel.app/)
- Backend (Render): (https://smartseason-field-monitoring-system-3kbd.onrender.com/)
- Database (Railway): Integrated via Render/Railway connection.
